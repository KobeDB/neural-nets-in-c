T_TestResultList test_neuron(Arena *arena) {
    ...
    NN_NeuronApplyResult n_result = nn_neuron_apply(scratch.arena, scratch.arena, &n, x);

    T_TestAssert(arena, &test_results, n_result.parameters.count == input_dim+1);

    F64 weighted_sum = bias;
    for (int i = 0; i < input_dim; ++i) weighted_sum += weights[i]*x_raw[i];
    T_TestAssert(arena, &test_results, n_result.neuron_output->value == weighted_sum);
...
T_TestResultList test_layer(Arena *arena) {
    ...
    NN_LayerApplyResult lresult = nn_layer_apply(scratch.arena, scratch.arena, &layer, x);

    T_TestAssert(arena, &test_results, lresult.parameters.count == ArrayCount(w0_raw)+ArrayCount(w1_raw)+1+1);
    ...
    T_TestAssert(arena, &test_results, lresult.layer_outputs.values[0]->value == expected[0]);
    T_TestAssert(arena, &test_results, lresult.layer_outputs.values[1]->value == expected[1]);
